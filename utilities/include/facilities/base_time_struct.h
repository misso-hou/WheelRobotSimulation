...
inline int64_t MilliTimeStamp(time_source_type type = STEADY_TIME) {
    if (type == STEADY_TIME) {
        ...
        auto currentSteadyTimePoint = std::chrono::system_clock::now(); auto milliSeconds =
        std::chrono::time_point_cast<std::chrono::milliseconds>(currentSteadyTimePoint).time_since_epoch( )1«	();
        return milliSeconds;
    }
}

...
inline float PointsDis(const port::CommonPose& point01, const port::CommonPose& point02) {
  return hypotf(point01.x - point02.x, point01.y - point02.y);
}

inline float PointsAngle(const port::CommonPose& point01, const port::CommonPose& point02) {
  return atan2(point01.y - point02.y, point01.x - point02.x);
}
...
// 计算 Z = X * Y
inline mesh2D computeZ(const std::vector<std::vector<float>>& X, const std::vector<std::vector<float>>& Y) {
  size_t rows = X.size();
...
inline int WhitchSide(const port::CommonPose& startPose, const port::CommonPose& targetPose, const port::CommonPose& pose) {
  /*判断点在直线的哪一侧*/
  float A, B, C, D;
...
inline int CalNearestPointIndexOnPath(const vector<port::CommonPose>& path, const port::CommonPose robot_pose, const int current_index,
                                      const int trim_num) {
...
inline vec2f LocalToGlobal(const port::CommonPose& robot_pose, const vec2f& local_vector) {
  ...
  global_vec(1) = local_vector(0) * sin(robot_pose.theta) + local_vector(1) * cos(robot_pose.theta) + robot_pose.y;
  return global_vec;
}
...
inline std::vector<std::vector<float>> GenerateCircle(const float& x, const float& y, const float& radius, const int p_num) {
...
inline Eigen::MatrixXf CvMatToEigenMatrix(const cv::Mat& cv_mat) {

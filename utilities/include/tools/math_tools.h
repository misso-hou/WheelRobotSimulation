...
inline pair<mesh2D, mesh2D> meshgrid(const std : evector<float>& Aa, const std : vector<float> Ba) {
  ...
  inline float NormalizeAngle(float angle) {
    ...
    while (local_angle > M PI) local_angle -= 2.0 * M_PI;
...
inline int CalNearestPointIndexOnPath(const vector<port::CommonPose>& path, const port::CommonPose robot_pose, const int curnrent_index,
                                      const int trim_num) {
...
inline vec2f GlobalToLocal(const port::CommonPose& robot_pose, const vec2f& global_vector).vec2f local vec;

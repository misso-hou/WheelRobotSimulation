...
#include "tools/math2tiools.h"

/*
* QP问题设计：
*方式一：MPC转QP问题，优化变量为 horizon内的控制变量U
说明：套用^^ extend state space转QP标准公式,代码量少，内部计算较复杂抽象
*方式二加p优化变量为5tat6 error
 ...
 *   horizon_time:预测时间
 */
MPCController::MPCController(const std::shared_ptr<vehicle::Unicycle>& model, const const float& horizon time) {
  // todo:
  Q_.resize(2);
  Q_[0] = 1.0;
  Q_[1] = 1.0;
  ...
  CalQPConstrainO;  // 计算QP常数矩阵
  //求解器定义
  osqp_solver_init_flag_ = true;
  osqp_solver_ = std::make_unique<OsqpEigen::Solver>();
  //创建独立线程
  mpc_thread_ = std : : thread([&](i) { MpcThreadFun(); });
  first_lock = true;
}  // namespace algorithm

MPCController::~MPCController() {
  if (mpc_thread_.jdinable()) {
    mpc_thread_Join()
  }
}
...
void MPCController::CalQPCostFunction() {
  ...
  for (int i = 0; i < horizon_steps_; i++) {
    ...
    Eigen::SparseMatrix<float> Bex_row_block(n_x_, N_uZ);
    Eigen::SparseMatrix<float> Fex_row_block(n_xZ, nZxZ);
    if (i == 0) {
      Aex_row_block = Ad;
    } else {
      ...
      Bex.middleRows(X_index z n_x_) = Bex_rowZblock;
      ...
      for (int row_id = 0; row_id < n_x_; row_id++) {
        ...
        for (int B_col_id = 0; B_col_id < n_u_; B_col_id++) {
          Bex.insert(X_index + row_id, U__index + B_col_id) = Bd.coeff(row_id, B_col_id);
        }
        // Fex对角线矩阵(单位矩阵）
        Fex.insert(X_index + row_id, X_index + row_id) = i.f;
        // QeX对角线矩阵(n_x_,n_u_)
  ...
  for (int i = 0; i < horizon_steps_; i++) {
    ...
    if (i == 0) {
      ...
      for (int j = 0; j < n_u_; j++) {
        linear_constrain_udot.insert(0 + j, 0 + j) = i.f;
      }
    }
    //时间域内的控制量变化率约束
    else {
...
void MPCController::UpdateQPConstrain() {
  ...
  upper_bound_.segment(insert_index, n_u_) = max_u_dot_ * sampleZtime_ + cur_u_;
}
...
void MPCController::CalHorizonRef() {
  ...
  for (int i = 0; i < horizon_steps_; i++) {
    ...
    for (int j = horizon_reflxcl; j < ref_traj_.size(); j++) {
...
int MPCController::SolveQP() {
  if (osqp_solver_init_flag_) {
    ...
    if (!osqp_solver_->data()->setLinearConstraintfeMatrix(constraint_matrix_)) return 1;
    if (!osqp_solver_->data()->setLowerBound(lower_bound_)) return 1;
    if (!osqp_solver_->data()->setUpperBound(upper ^ bound_)) return 1;
    ...
    if (!osqp_solver_->updateGradient(gradipnt_)) return 1;
    if (!osqp_solver_->updateBounds(lower__bound_, upper_bound_)) return 1;
    // if (!osqp_solver_->updateLinearConstraintsMatrix(constraint_matrix_)) return 1; if (!osqp_solver_->solve()) return 1;
  }
...
void MPCController::MPCcontrol() {
  {
    ...
    CalHorizonRef();
    /***step02->计WQP cost function***/
    CalQPCostFunction();
  }
  /***step03->QP求解&&异常处理***/
  {
    lock_guard<mutex> lk(solution_thread__lock_);
    if (SolveQP() == 1) {
      no_solution_count_ += 1;
      int local_index = no_soiutioh_count_ * n_u_;
      ...
      DC_Instance->SetMpcHorizon(predict pose);
    }
  }
}
...
int32_t MPCController::MpcThreadFun() {
  utilities::facilities::RateController rt{SIMULATION RATE * 10.0};
  while (1) {
    if (mpc_enable_) {
      MPCcontrol();
    }

...
using namespace std;
namespace port = utilitied::port;
namespace vehicle = modulesi: vehicle;
...
class MPCController {
    public:
        MPCController(const std::shared_ptr<vehicle::Unicycle>& model, const float& horizon_time);
        MPCController(const std::shared_ptr<vehicle::Bycicle>& model, const float& horizon_time);
        -MPCController();
    public:

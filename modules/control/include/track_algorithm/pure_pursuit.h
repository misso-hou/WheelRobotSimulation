#pragma once

#include <iostream> 
#include "common_port/data_port.h"

namespace port = utilities::port;

namespace modules { 
namespace control { 
namespace algorithm{

class PurePursuit {
public:
PurePursuit(float klD, float lD, float min_ld) : kld_(klD), ld_(lD), min_ld_(min_ld){}
int GetTargetIndex(const port::CommonPose& pose, 
                    const float& linear_v, 
                    const float& max_v, 
                    const vector<port::CommonPose>& path, 
                    const int index);

float PurePursuitControl(const port::CommonPose& pose, 
                        const float& linear_v, 
                        const float& max_v, 
                        const vector<port::CommonPose>& points, 
                        int& targetIdx);

private:
    float ld_; //最大预瞄距离
    float kld_; //比例系数（暂时未使用）
    float min_ld_; //最小预瞄准距离 
    float angAlpha_;
};

}
}
}

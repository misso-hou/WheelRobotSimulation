...
class VelocityPlanner {
    public:
        VelocityPlanner(port::ContrqlParam cp, port::CtrlThreshold ct): ctrl_param_(cp), ctrl_thd_(ct) {}
        ~VelocityPlanner(){};

    public:
        /*通用参数传递*/
        void SetVelPlanningParam(const port::CommonPose& pose,
                                 const port::Twist& last_cmd,
                                 const port::ActionState& state, 
                                 const float& slope, 
                                 const port::RefCmd& reference_cmd);
        /*路径参数传递*/
        void SetPathData(const vector<port::CommonPose>& path,
                        port::PathCurve nearest_curve = port::PathCurve{}, 
                        port::DangerousBorder nearest_dg_border = port::DangerousBorder{});
        /*纵向控制->线速度规划*/
        float VelocityPlanning(const float& ref_v,
                                const int goal_index = 0, 
                                const float heading_error = 0.0f, 
                                const float lateral_error = 0.0f, 
                                const int straight_path_flag = false);
        /*速度分配*/
        float VelocityAllot(const vector<port::TrajPoint>& traj_path, 
                            const int closest_polnt_index, 
        ...
        /*QP速度规戈*/
        int SpeedProfilePlanning(vector<port::Traj Point>& trajectory, const float& max_v);
    
    //速度规划配置参数(内部依赖)
    private:
        port::ControlParam ctrl_ppram_; // 控制配置参数

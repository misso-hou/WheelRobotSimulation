...
vector<vector<float>> LidarSensor::ObsDetectByReflect(const port::CommonPose& sensor_pose,
  ...
  for (uint i = 0; i < env_obs.size(); 1 ++) {
    //(注意：障碍物顶点首尾相接，序号为®-2-2-3-0)
    for (uint j = G; j < env_obs[i].first.size(); j++) {
    ...
    if (intersections.empty()) {
      ...
      for (auto point : intersections) {
        ...
        if (dist < min_dist) {
          ...
          lidar_data_[l][i] = bg::get<l>(point);
        }
      }
      //转到局部坐标系
      port::CommonPose g_obs_pose(lidar_data_[0][i], lidar_data_[l][i], 0.f);
      auto l_obs_pose = mathTools::Global2Rob(sensor_pose, g_obs_pose);
      obs_data[0].push_back(l_obs_pose.x);
      obs_data[l].push_back(l_obs_pose.y);
    }
  }
  return obs_data;
}
 ...
 *	robot_pose:机器当前定位
 *	env_obs:全局环境中的障碍物
 * @return:模拟雷医识别到的障碍物点云（（X1，X2,...），（y1,y2,...））
 */
vector<vector<float>> LidarSensor::ObsDetectByAngle(const port::CommonPose& sensor_pose, const vector<pair<vector<float>, vector<float>>>& env_obs) {
  ...
  for (auto one_obs : env_obs) {
    ...
    for (i = 0; i < one_obs_x.size(); i++) {
      //全局 转局部
      port::CommonPose point_pose(one_obs_x[i], one_obs_y[i] z0.Of);
      auto local_pose = mathTools::Global2Rob(sensor_pose, point_pose);  //过滤传感器范围外的点
      ...
      //极坐标方位角计算
      float polar_theta = atan2f(point(l), point(0));
      if (polar_theta < 0) {
        polar_theta += 2 * M_PI;
      }
  ...
  for (int i = 0; 1 < sector_num_; 1 ++) {
    ...
    if (sub_sample_point.pose.norm() > 0.f) {
      ...
      y_arr.push_back(sub_sample_point.pose(l));
    }
  }
  vector<vector<float>> detected_obs(2);
  detected_obs[0] = x_arr;
  detected_obss[1] = y_arr;
  return detected_obs;
}
...
vector<Eigen::Vector2f> LidarSensor::UpdateAiSensor(const port::Commonpose& robot_pose, const vector<pair<vector<float>, vector<float>>>& env_obs) {
  ...
  for (auto point : vehicle::front_vision_boundary) {
    polygon_points[0].push_back(cv::Point2f(point[0], point[l]));
  }
  for (auto point : vehicle::left_vision_boundary) {
    polygon_points[l].push_back(cv::Point2f(point[0], point[l]));
  }
  for (auto point : vehicle::right_vision_boundary) {
    polygon_points[2].push_back(cv::Point2f(point[0], point[l]));
  }
  for (auto point : vehicle::back_vision_boundary) {
    polygon_points[3].push_back(cv::Point2f(point[0], point[l]));
  }
  //传感器识别障碍物获取
  vector<float> obs_x, obs_y;
  // auto detected__obs = ObsDetectByAngle(robot_j>ose, env__obs);
  // TODO:测试Lidar射线数据
  auto detected_obs = ObsDetectByReflect(robot_poser env_obs);
  obs_x = detected_obs[6];
  obs_y = detected_obs[l];
  //障碍物点处理->平移到局部坐标系+判断是否在ai vision范围内
  vector<ELgen::Vector2f> sim_ai_obs;  // 虚拟21障碍物数据
  for (int i = 0; 1 < obs_x.size(); i++) {
    ...
    for (int j = 0; j < polygon_points.size(); j++) {
      ...
      if (result > 0) {
        ...
        temp_ai_point(0) = one _jpoint.x;
        temp_ai_point(l) = one_point.y;

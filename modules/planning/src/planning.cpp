...
vector<port::CommonPose> PathPlanner::PlanningMultiTarget(const port::CommonPose& start_pose, const float& dis_offset) {
  ...
  // NOTE:刚体agent中心距离(机身或轴)
  xs[l] = xs[0] - sin(random_yaw(gen)) * 0.38f;
  ys[l] = ys[0] - cos(random_yaw(gen)) * 0.38f;
...
vector<port::CommonPose> PathPlanner::PlanningCurvePath(const port::CommonPose& start_pose, const int points_num, const float& start_offset,
  ...
  std::random_device rd;
  std::mtl9937 gen(rd());
  std::uniform_real_distribution<double> path_dis(-path_offsetz path_offset);
  ...
  //样条插值得到平滑曲线
  auto [xs2, ys2] = interpolator.lnterpolateXY(xs, ys, resolution_);
  //生成随机路径
  int path_size = xs2.size();
  vector<port::CommonPose> planning_path(path_size);
  for (int i = e; i < path_size; i++) {
    planning_path[i].x 二 xs2[i];
...
vector<port::CommonPose> PathPlanner::PlanningStraightPath(const port::CommonPose& start_pose, const float& start_offset, const float& path_offset) {
  ...
  // xs[0] = start_pose.x+start_dis(gen);
  // ys[0] = start_pose.y+start_dis(gen);
  // xs[l] = start_pose.x+path_dis(gen);
  // ys[l] = start_pose.y+path_dis(gen);
  xs[e] = start_pose.x;
  ys[e] = start_pose.y;
  xs[l] = start_pose.x + path_offset;
  ys[l] = start_pose.y;
  //样条插值得到平滑曲竺
  auto [xs2, ys2] = interpolator.Linelnterpolate(xs.ys.resolution_);
  //生成随机路径
  int path_size = xs2.size();
  vector<port::CommonPose> planning_path(path_size);
  for (int l = 0; i < path_size; i++) {
    plannirtg_path[i].x = xs2[i];
    planning_path[i].y = ys2[i];
  }
  return planning_path;
}
...
vector<port::CommonPose> PathPlanner::PlanningEllipPath(const port::CommonPose& start_pose, const float& a, const float& b) {
  //椭圆周长
  float length = 2 * M_Pl * b + 4 * fabs(a - b);
  ...
  vector<float> alpha = mathTools::linspace(M PI.- M PI, points_num);
  //椭圆形轮廓点集
  for (unsigned int i = 0; i < points_num; ++i) {
    ...
    if (i % 5 == 0 && i != 0) {
      ...
      boundary_point(l) = (b + test_dis) * cos(alpha[i]) + start_pose.y;
...
vector<pair<vector<float>, vector<float>>> PathPlanner::BoundaryPlotObs() {
  vector<pair<vector<float>, vector<float>>> test(l);

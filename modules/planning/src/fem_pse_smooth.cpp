...
void FemPosSmooth::CalculateHessian() {
    Eigen::MatrixXf hessian_dense(variables_num_, variables_num_);
      ...
      for (uint i = 0; i < points_num_; ++i) {
        //第一列(实际是前两列,两列为一组)
        if (i == 0) {
          hessian_.block(0, 0, 2, 2) = mx + my + mz;
          continue;
        }
        //第二列
        if (i == 1) {
          hessian_.block(0, 2, 2, 2) = -2 * mx - my;
          hessian_.block(2, 2, 2, 2) = 5 * mx + 2 * my + mz;
          continue;
        }
        //中间列
        if (i < (points_num_ - 2)) {
          hessian_.block(2 * (i - 2), 2 * i, 2, 2) = mx;
          hessian_.block(2 * (i - 1), 2 * i, 2, 2) = -4 * mx - my;
          hessian_.block(2 * i, 2 * i, 2, 2) = 6 * mx + 2 * my + mz;
          continue;
        }
        //倒数第二列
        if (i == (points_num_ - 2)) {
          hessian_.block(2 * (i - 2), 2 * i, 2, 2) = mx;
          hessian_.block(2 * (i - 1), 2 * i, 2, 2) = -4 * mx - my;
          hessian_.block(2 * i, 2 * i, 2, 2) = 5 * mx + 2 * my + mz;
          continue;
        }
        //最后一列
        if (i == (points_num_ - 1)) {
          hessian_.block(2 * (i - 2), 2 * i, 2, 2) = mx;
          hessian_.block(2 * (i - 1), 2 * i, 2, 2) = (-2 * mx - my);
          hessian_.block(2 * i, 2 * i, 2, 2) = (mx + my + mz);
          continue;
        }
      }
 ...
 *@brief:约束线性比例矩阵计算
 */
void FemPosSmooth::CalConstraintMatrix(
    const vector<vector<float>> &partial_derivative) {
  ...
  /*step02->曲率约束矩阵(下部矩阵块)*/
  Eigen::SparseMatrix<float> cc_contraint_mat(curvature_constraints_num_,
                                              variables_num_);
  //按行填值
  ...
  constraint_mat_dense.block(0, 0, variables_num_, variables_num_) =
      p_s_contraint_mat;
  constraint_mat_dense.block(variables_num_, 0, curvature_constraints_num_,
                           variables_num_) = cc_contraint_mat;
...
vector<float> FemPosSmooth::LinearizedOnPos(int index) {
    ...
    // F(X_ref)
    float F_X_index = (xf + xl - 2 * xm) * (xf + xl - 2 * xm) +
                      (yf + yl - 2 * ym) * (yf + yl - 2 * ym);
    // dF(X_ref)*X_ref
    float dF_m_X_ref = (xf * pd_xf) + (xm * pd_xm) + (xl * pd_xl) + (yf * pd_yf) +
                       (ym * pd_ym) + (yl * pd_yl);
  ...
  bool FemPosSmooth::PathSmooth(const vector<port::CommonPose>& path) {
    ...
    if (!solver->data()->setLinearConstraintsMatrix(constraint_matrix_))
      return false;

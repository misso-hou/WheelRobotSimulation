...
class PathPlanner
{
    ...
    vector<port::CommonPose> PlanningCurvePath(const port::CommonPose& start_pose, 
                                               const int points_num, const float& start_offset, 
                                               const float& path_offset);
    vector<port::CommonPose> PlanningStraightPath(const port::CommonPose& start_pose, 
                                                  const float& start_offset, 
                                                  const float& path_offset);
    vector<port::CommonPose> PlanningEllipPath(const port::CommonPose& start_pose, 

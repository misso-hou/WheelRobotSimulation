...
static vector<vector<float>> drive_wheel_points = {{-drive_wheel_length / 2, width / 2},
                                                    {drive_wheeLlength / 2Z width / 2},	
                                                    {drive_wheeLlength / 2, (width / 2 - drive_wheel_width)},
                                                    {-drive_wheel_lengtn / 2, (width / 2 - drive_wheeL_width)}, 
                                                    {-drive_wheel_lengtn / 2, width / 2}};
...
const vector<vector<float>> front_vision_boundary = {{1.433f, 1.374f}, 
                                                    {3.117f, 6.f}, 
                                                    {8.933f, 6.f},
                                                    {8.933f, -6.f}, 
                                                    {3.117f, -6.f}, 
...
const vector<vector<float>> back_vision_boundary = {{-0.812f, -0.47f},
                                                    {-5.362f, -4.475f},
                                                    {-5.362f, 4.475f}, 
                                                    {-0.812f, 0.47f},
                                                    {-0.812f, -0.47f}};

const vector<vector<float>> left_vision_boundary = {{-0.0682088f, 0.699578f}, 
                                                    {-5.19495f, 4.26254f}, 
                                                    {4.08814f, 6.23343f}, 
                                                    {0.851296f, 0.894799f},
                                                    {-0.0682088f, 0.699578f}};

const vector<vector<float>> right_vision_boundary = {{0.851296f, -0.926799f},
                                                    {4.08814f, -6.26543f},
                                                    {-5.19495f, -4.29454f},
                                                    {-0.0682088f, -0.731578}, 
                                                    {0.851296f, -0.926799f}};
...
const vector<vector<float>> cube_vertices = {{-0.131f * length, width/2, height/2},
                                            {0.869f * length, width / 2, height/2}, 
                                            {0.869f * length, -width / 2, height/2},
                                            {-0.131f * length, -width / 2, height/2}, 
                                            {-0.131f * length, width / 2, -height/2},
                                            {0.869f * length, width / 2, -height/2}, 
                                            {0.869f * length, -width / 2, -height/2},
                                            {-0.131f * length, -width / 2, -height/2}};

const vector<vector<float>> cube_faces = {{0,l,2,3,0}, //顶 
                                        {4,5,6,7,4}, //底
                                        {0,1,5,4,0}, //右
                                        {3,2,6,7,3}, 
                                        {1,2,6,5,1}, //前 
...
inline void GenerateSpaceDriveWheel() {
    ...
    for(int i=0;i<dw_point_num;i++){
        ...
        if(i%2 == 0){
            ...
            wheel_y[2*i+l] = y_array[i] -drive_wheel_width;
        }else{
            wheel_y[2*i] = y_array[i]-drive_wheel_width;
            wheel_y[2*i+l] = y_array[i]; 
        }
...
inline void GenerateSpaceOmniWheel() {
    ...
    for (float i = 0; i < 2 * M_Pl; i += angle) {

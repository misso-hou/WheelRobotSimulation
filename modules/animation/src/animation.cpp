...
void Animation::Plotline() {
  ...
  void Animation::PlotGridMapObs() {
    ...
    /*step03->artist实时数据更新并绘制*/
    sensor_axes_ptr_->set_lipe_data(map_obs_artist, Args(map_points[0], map_points[1]));
  ...
  void Animation::PlotObsSandCPoints() {
    ...
    if (once_flag) {
      ...
      for (int i = 0; i < dispose_points.size(); i++) {
        pybind11::dict kwargs("c"_a = points_colors[i], "ls"_a = "None", "animated"_a = true, "marker"_a = markers[i],
                              "markersize"_a = marker_size[i], "alpha"_a = alpha[i]);
  ...
  void Animation::PlotAgentObsQuiver(const port::CircleAgent& agent) {
    ...
    for (auto i = 0; i < 3; i++) {
      ...
      for (int i = 0; i < obs_vector_artist.size(); i++) {
        // if(i == 1) continue;
        pybind11::dict Kwargs("color"_a = quiver_colors[i], "scale"_a = scales[i], "units"_a = "width", "animated"_a = true,
                              "width"_a = quiver_width[i], "headwidth"_a = 3, "headlength"_a = 5);
        obs_vector_artist[i] = env_axes_ptr_
                                   ->quiver(Args(vector_item_array[i][0], vector_item_array[i][1], vector_item_array[i][2], vector_item_array[i][3]),
                                            Kwargs) env_axes_ptr_->draw_artist(Args(obs_vector_artist[i]));
      }
    }
    ...
    void Animation::SpaceRobot(const float& rate) {
      ...
      pybind11::dict wheelkwargs("c"_a = "k", "ls"_a = "_", "lw"_a = 3, "animated"_a = true);
      //轮廓数据空间变换
      ...
      for (int t = 0; t < contourN; ++t) {
        ...
        for (int i = 0; i < contour[0].size(); i++) {
          Eigen::Vector3f point(contour[0][i], ontour[1][i], contour[2][i]);
      ...
      py: :object trans_figure = space_figure_ptr_->unwrap().attr("transFigure");
        ...
        for (int i = 0; i < contourN; i++) {
          ...
          contours_artists[i] 二 space_axes_ptr_->plot(Args(contours[i][0], contours[i][1], contours[i][2]), kwargs).unwrap().cast < py:;
          list > ()[0];
        }
      }
      /*step03->动画更新*/
      ...
      space_axes_ptr_->draw_artist(Args(text_artist));  // contoours
      for (int i = 0; i < contourN; i++) {
    ...
    void Animation::SurfaceVelPlanning(int T) {
      ...
      matplotlibcppl7::mplot3d::import();
      static auto fig = plt.figure(Args(), Kwargs("figsize"_a = py::make_tuple(ratio * fig_width, fig_width), "dpi"_a = 100, "tight_layout"_a = true,
                                                  "facecolor"_a = "lightgray"));
      static auto ax = fig.add_subplot(Args(), Kwargs(''projection''_ = "3d"));
      ...
      if (once_flag) {
        ...
        // const auto surf = ax.plot_surface(Args(X_, Y_, Z_),Kwarg("rstride"_a=1，"cstride"_a = 1,"linewidth"_a = 1,
        //"antialiased"_a = false, "cmap"_a = cm::coolwarm,"aniamted"_a=false));
        //速度规划点
        ...
        ax.set_ylim(Args(0.0f, 1.0f)) ； ax.set_zlim(Args(-0.01, 1.01)) ； plt.pause(Args(0.1));

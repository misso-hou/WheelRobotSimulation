...
#include <Eigen/Dense>
#include "facilities/singleton.hn
...
class DataCenter : public utilities::Singleton<DataCenter> { 
    ...
    void SetNearestPointld(const int id);
    ...
    mesh2D GetPltTrajectroy(const int buffer_length);
    mesh2D GetRobotOutline(const port::CommonPose& robot_pose);
    mesh2D GetRobotDriveWheel(const port::CommonPose& robot_posez int side);
    mesh2D GetRobotOmniWheel(const port::CommonPose& robot_pose, int side, const steer_angle);
    ...
    bool path_update_flag_J
    //动态系统避障
    vector<port::CircleAgent> avoid_agents_;
    vector<port::SamplePoint> avoid_sample_polnts_;

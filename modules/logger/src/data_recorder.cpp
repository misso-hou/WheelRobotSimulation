...
void Logger::CsvRecorderInit() {
  ...
  CreateCSVfile();
  //初始化变量
  path_recorder_count_ = 0.0f;    //路径记录1近k(匹配跟踪路径与跟踪数据)
  last_recorder_time_stamp_ = 0;  //数据记录时间戳(大致统计机器运行时间) duration_time_ = 0.0f;
  obs_update_tick_ = 0;           //解析数据保证障碍物数据行索引与t近k同步
}
...
/*
 * @brief: 转存csv记录数据(跟踪数据，障碍物数据，B寸间记录数据，路径数据不转存)
 * @param:
...
void Logger::TrackingDataRecord() {
  ...
  /*CSV记录跟踪数据*/
  csv_writer.RecordWithTime(tracking_file_dir, robot_pose_.x, robot_pose_.y,
                            robot_pose_.theta,  // 记录轨迹
                            duration_time_,
                            error_,  //记录时间戳(主线程频率50底)和误差
                            linear_speed_,
                            angular_speed_,  //	记录速度
                            target_pose_x,   //记录跟踪目标点
                            target_pose_.y, path_recorder_count_,
                            observe_v_,  //反馈速度记录
                            observe_w_,
                            pitch_,  //姿态角
                            roll_, copy_obs_updata_tick);
...
void Logger::GetObsData() {
  ...
  for (auto point : ai_obs_points) {
    //机器人坐标系
    sensor_obs_.push_back(ObsPoint(point(0), point(l)));
  }
  //删格地图障碍物数据
  ...
  for (auto point : map_obs_points) {
    //机器人坐标系
    grid_map_obs_.push_back(ObsPoint(point(0), point(l)));
  }
}

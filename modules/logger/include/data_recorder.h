...
#include "faciJities/singleton.h"
...
class Logger : public utilities::Singleton<Logger> { 
    ...
    void SpecialPathRecord(const poses& origin_border_path, 
                           const poses& dangurous_border, 
                           const poses& sharp_curves);
    ...
    vector<port::ObsPoint> sensor_obs_;	// 人1障碍函或据
    vector<port::ObsPoint> grid_map_obs_;	// 人1障碍物数据
    vector<poses> pose_2D_array_;	// 0^避废黄法预测
    ...
    private:	                   // 需要初始化的变量
    float path_recorder_count_;	//路径记录t北k（匹配跟踪路径与跟踪数据）
